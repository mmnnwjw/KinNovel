...
# -EPDC 常量(mxcfb:KOReader mxcfb-kindle.h,Kindle lab126 内核)
# mxcfb 波形模式
class WAVEFORM:
    INIT = 0x0
    DU = 0x1
...
class FLAG:
    ...
    USE_DITHERING_Y2 = 0x4000
    USE_DITHERING_Y4 = 0x8000
    USE_DITHERING = 0x2000 | 0x8000
...
def _mxcfb_ioctls():
    return {
        "send_update": _iow("F", 0x2E, ctypes.sizeof(MxcfbUpdateData)),
...
class EInkDisplay:
    ...
    def mxc_update(self, x, y, w, h, is_flashing, waveform_mode,
        ...
        # 1. 边界对齐:起点向下对齐,终点向上对齐,避免边缘像素永不刷新
        right = min(self.width, x + w)
        bottom = min(self.height, y + h)
        x = (x // alignment) * alignment
        y = (y // alignment) * alignment
        w = min(self.width - x, ((right - x + alignment - 1) // alignment) * alignment)
        h = min(self.height - y, ((bottom - y + alignment - 1) // alignment) * alignment)
        if w <= 0 or h <= 0:
            return None
    ...
    def probe(self):
        return self.mxc_update(0, 0, 16, 16, False, self.W.GC16) is not None
